...
FROM
    ...
    CLASS C
WHERE
    S1.USN = C.USN AND C.SSID = S2.SSID
        AND S2.SEM = 4
        AND S2.SEC = 'C';
...
in each section. */
SELECT 
    S.GENDER, SS.SEM, SS.SEC, COUNT(GENDER)
FROM
    STUDENT S,
    SEMSEC SS,
    CLASS C
WHERE
    S.USN = C.USN AND C.SSID = SS.SSID
...
UPDATE IAMARKS 
SET 
    FINALIA = ((GREATEST(TEST1, TEST2, TEST3) + ((TEST1 + TEST2 + TEST3) - GREATEST(TEST1, TEST2, TEST3) - LEAST(TEST1, TEST2, TEST3))) / 2);
SELECT 
    *
FROM
    IAMARKS;
...
FROM
    ...
    COURSE SUB
WHERE
    S.USN = IA.USN AND SS.SSID = IA.SSID
